...
import { fetchMailData, readMessage } from "../../store/mailAllActions";
        ...
        <section className="border-2 border-amber-900 bg-amber-50 rounded-lg my-6 w-3/4 h-full m-auto shadow-[0_0_40px_-10px_rgba(0,0,0,0.6)] shadow-amber-900">
            {receivedMails && receivedMails.length > 0 ? (
                receivedMails.map(mail=>(
                    <div key={mail.id} onClick={()=>{readMessageHandler(mail.id)}} className="border-b-2 cursor-pointer flex justify-evenly border-slate-500 w-full h-12 p-2 rounded-lg bg-slate-300">
                        <div className="flex gap-2">
                            {!mail.isRead && <img src="assets/unread.png" alt="Unread mail." className="w-3 h-3 m-auto mr-2" />}
                            {mail.from}
                        </div>
                        <div className="flex gap-2">
                            {mail.subject}
